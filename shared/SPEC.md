...
- **Cursor dot in light mode (2026-09-18):** the `difference` blend sat on the inner ring, whose backdrop is the transparent outer box, so it never blended with the page and the dot was white in light mode. The blend now lives on the outer `.cursor` (root stacking context); `.is-hot` switches it back to normal. Verified: dot 10/10/13 on the light ground, 235/233/229 on the inverse ground; white on dark, dark on the dark theme's light Contact.
- **Contact links (2026-09-18):** "Copy" gives feedback in three places: the button fills in the accent and reads "Copied ✓" for two seconds, the cursor bubble (which covers the button on desktop) re-reads the label through a `cursorrefresh` event, and the button is `aria-live`. Clipboard API with a textarea/`execCommand` fallback; if both are blocked, the address is selected for manual copy. LinkedIn, the résumé and the GitHub source open in a new tab (`target="_blank"`, `rel="noopener noreferrer"`, ↗ mark). The résumé is `site/public/andre-ferreira-resume.pdf`, linked through `href()` so postbuild keeps it relative.
